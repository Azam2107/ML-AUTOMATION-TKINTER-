...
def model():
    ...
    x_train,x_test,y_train,y_test = train_test_split(x, p, test_size=float(splits.get()))
    ...
    model = GridSearchCV(boost, param_grid=parameters, scoring = 'accuracy')
    ...
    Label(root, text=str(le.inverse_transform(y_pred)), font=('Helvetica', 10, 'bold'), bg="light blue", relief="solid").place(x=400,
                                                                                                         y=450)
...
def files():
    with open(r"C:\Users\musan\Desktop\model summary", "w", encoding="utf-8") as file:
        file.write("You have used Best_classifier model \n")
        file.write("\n")
...
Button(root, text="Summary", command=files,activeforeground="white",activebackground="black").place(x=490, y=190)
