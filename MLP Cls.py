...
def model():
    ...
    model=MLPClassifier(activation=activ.get(),solver=solve.get(),learning_rate=learn.get())
...
def files():
    with open(r"C:\Users\musan\Desktop\model summary", "w", encoding="utf-8") as file:
        file.write("You have used MLP Classifier \n")
        file.write("\n")
        file.write(f"The columns used for features are {feature_col} and the targetted columns are {target_col}\n")
        file.write("\n")
        file.write(
            f"The Hyper parameters used in the model are initiated as Activation - {activ.get()} , Solver - {solve.get()} , Learner - {learn.get()}\n")
        file.write("\n")
        file.write(
...
activ = StringVar()
activ.set("")
choose = ttk.Combobox(root, width=30, textvariable=activ)
choose['values'] = ("identity", "logistic", "tanh", "relu")
choose.place(x=250, y=300)
Label(root, font="System", text="Choose the activation").place(x=20, y=300)

solve = StringVar()
solve.set("")
choose = ttk.Combobox(root, width=30, textvariable=solve)
choose['values'] = ( "lbfgs", 'sgd', "adam")
choose.place(x=250, y=330)
Label(root, font="System", text="Choose the solver").place(x=20, y=330)


learn = StringVar()
learn.set("")
choose = ttk.Combobox(root, width=30, textvariable=learn)
choose['values'] = ("constant", "invscaling", "adaptive")
choose.place(x=250, y=360)
Label(root, font="System", text="Choose the learning rate").place(x=20, y=360)
