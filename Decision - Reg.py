...
root= Tk()
root.title('Decision tree Regressor')
...
def model():
    ...
    from sklearn.tree import DecisionTreeRegressor
    model=DecisionTreeRegressor(criterion=criterion.get(),splitter=splitter.get(),max_features=max_f.get())
...
def files():
    with open(r"C:\Users\musan\Desktop\model summary","w",encoding="utf-8") as file:
        ...
        file.write("\n")
        file.write(f"The Hyper parameters used in the model are initiated as criterion - {criterion.get()} , splitter - {splitter.get()} , max_features - {max_f.get()}\n")
        file.write("\n")
...
Label(root,font="System",text="Criterion").place(x=20,y=330)
Label(root,font="System",text="splitter").place(x=20,y=360)
Label(root,font="System",text="max_feature").place(x=20,y=390)


Label(root,font="System",text="split_size",fg="black",bg="white",width=15,height=1,relief="sunken").place(x=20,y=300)
splits = StringVar()
choose = ttk.Combobox(root, width = 22, textvariable = splits)
choose['values'] = ('0.2','0.25','0.3')
choose.place(x=220,y=300)

criterion = tk.StringVar()
choose = ttk.Combobox(root, width = 22, textvariable = criterion)
choose['values'] = ('mse','friedman_mse','mae','poisson')
choose.place(x=220,y=330)

splitter = tk.StringVar()
choose = ttk.Combobox(root, width = 22, textvariable = splitter)
choose['values'] = ('best','random')
choose.place(x=220,y=360)

max_f = tk.StringVar()
choose = ttk.Combobox(root, width = 22, textvariable = max_f)
choose['values'] = ('auto','sqrt','log2')
choose.place(x=220,y=390)
