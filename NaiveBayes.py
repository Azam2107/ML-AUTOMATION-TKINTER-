...
root= Tk()
root.title('Multinomial - Naive Bayes')
...
def files():
    with open(r"C:\Users\musan\Desktop\model summary","w",encoding="utf-8") as file:
        file.write("You have used Mltinomial - Naive Bayes model \n")
        file.write("\n")
        file.write(f"The columns used for features are {feature_col} and the targetted columns are {target_col}\n")
        file.write("\n")
        file.write(f"The Hyper parameters used in the model are initiated as Alpha - {float(alph.get())} , fit_prior - {bool(fit_p.get())}\n")
        file.write("\n")
...
Label(root,font="System",text="fit_prior").place(x=20,y=360)


alph=tk.StringVar()
choose=ttk.Combobox(root,width=30,textvariable=alph)
choose['values']=('1','2','3','4')
choose.place(x=200,y=330)


fit_p=tk.StringVar()
choose=ttk.Combobox(root,width=30,textvariable=fit_p)
