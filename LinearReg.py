...
def model():
    ...
    model=LinearRegression(fit_intercept=bool(int(fitin.get())),normalize=bool(int(normalz.get())))
...
def files():
    with open(r"C:\Users\musan\Desktop\model summary","w",encoding="utf-8") as file:
        file.write("You have use Linear Regression model \n")
        file.write("\n")
        file.write(f"The columns used for features are {feature_col} and the targetted columns are {target_col}\n")
        file.write("\n")
        file.write(f"The Hyper parameters used in the model are initiated as fit_intercept - {fitin.get()} , normalize - {normalz.get()}\n")
        file.write("\n")
...
Label(root,font="System",text="fit_intercept").place(x=20,y=330)
Label(root,font="System",text="Normalize").place(x=20,y=360)
...
fitin = tk.StringVar()
choose = ttk.Combobox(root, width = 22, textvariable = fitin)
choose['values'] = ('0','1')
choose.place(x=220,y=330)

normalz = tk.StringVar()
choose = ttk.Combobox(root, width = 22, textvariable = normalz)
choose['values'] = ('0','1')
