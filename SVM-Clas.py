...
def model():
    ...
    model=SVC(C=float(Penalty.get()),kernel=Kernal.get(),decision_function_shape=dca.get())
...
def files():
    with open(r"C:\Users\musan\Desktop\model summary","w",encoding="utf-8") as file:
        file.write("You have used SVM classifier model \n")
        file.write("\n")
        file.write(f"The columns used for features are {feature_col} and the targetted columns are {target_col}\n")
        file.write("\n")
        file.write(f"The Hyper parameters used in the model are initiated as Penalty - {float(Penalty.get())} , Kernal - {Kernal.get()}, Decesion func shape - {dca.get()}\n")
        file.write("\n")
...
Label(root,font="System",text="Penalty").place(x=20,y=300)
Label(root,font="System",text="Kernal").place(x=20,y=330)
Label(root,font="System",text="Decision func shape").place(x=20,y=360)


Penalty=tk.StringVar()
choose=ttk.Combobox(root,width = 30, textvariable= Penalty)
...
dca =tk.StringVar()
choose=ttk.Combobox(root,width=30,textvariable= dca)
