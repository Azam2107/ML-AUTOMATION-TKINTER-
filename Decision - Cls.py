...
def data():
    ...
    for i in file.columns:
        if type(file[i]) == np.float64 or type(file[i]) == np.int64:
            file[i] = file[i].fillna(file[i].median(), inplace=True)
        elif type(file[i][0]) == type("_"):
...
def model():
    ...
    model = DecisionTreeClassifier(criterion=criterion.get(), splitter=splitter.get(), max_features=max_f.get())
...
def files():
    with open(r"C:\Users\musan\Desktop\model summary", "w", encoding="utf-8") as file:
        ...
        file.write("\n")
        file.write(
            f"The Hyper parameters used in the model are initiated as criterion - {criterion.get()} , splitter - {splitter.get()} , max_features - {max_f.get()}\n")
        file.write("\n")
        file.write(
...
criterion = StringVar()
choose = ttk.Combobox(root, width=30, textvariable=criterion)
choose['values'] = ("gini", "entropy")
choose.place(x=170, y=300)
Label(root, font="System", text="Choose the criterion").place(x=20, y=300)

splitter = StringVar()
choose = ttk.Combobox(root, width=30, textvariable=splitter)
choose['values'] = ("best", "random")
choose.place(x=170, y=330)
Label(root, font="System", text="Choose the splitter").place(x=20, y=330)

max_f = StringVar()
choose = ttk.Combobox(root, width=30, textvariable=max_f)
choose['values'] = ("auto", "sqrt", "log2")
choose.place(x=170, y=360)
Label(root, font="System", text="Choose the features").place(x=20, y=360)
...
choose.place(x=170, y=390)

s = tk.StringVar()   
Entry(root,text=s,width=30).place(x=250,y=450)
Label(root,font="System",text='Inputs separated by commas').place(x=20,y=450)
Button(root, text="Plot", activeforeground="white",activebackground="black",command=plot).place(x=600, y=50)
...
Button(root, text='Select X', command=getx,activeforeground="white",activebackground="black").grid(row=14, column=1)

box3 = Listbox(root)
box3.grid(row=10, column=2)
Button(root, text='Select Y', command=gety,activeforeground="white",activebackground="black").grid(row=14, column=2)

Button(root, text="predict", command=model,activeforeground="white",activebackground="black").grid(row=14, column=3)
